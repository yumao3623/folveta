...
export async function cancelCurrentSubscription() {
  const user = await getCurrentUser();
  if (!user) return { error: "Not authenticated" } as const;
  const { data: subscription } = await getSupabaseAdmin().from("billing_subscriptions").select("paddle_subscription_id").eq("user_id", user.id).eq("billing_environment", getBillingEnvironment()).in("status", ["active", "trialing", "past_due"]).order("updated_at", { ascending: false }).limit(1).maybeSingle();
