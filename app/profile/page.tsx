    ...
    <WorkspaceShell active="profile">
      <div className="mx-auto w-full max-w-[800px]">
        <header className="border-b border-[var(--border)] pb-7">
          ...
          <h1 className="mt-2 font-display text-[36px] font-bold leading-tight text-[var(--foreground)] sm:text-[44px]">Profile</h1>
        ...
        <section className="mt-7 ui-surface ui-surface--base p-5 sm:p-6" aria-labelledby="account-details-heading">
          ...
          <dl className="mt-5 divide-y divide-[var(--border-soft)]">
            <div className="flex min-w-0 items-center gap-3 py-4 first:pt-0"><StudyIcon name="profile" size={26} /><dt className="w-28 shrink-0 text-[13px] text-[var(--muted)]">Email</dt><dd className="min-w-0 break-all text-[14px] font-medium">{user.email}</dd></div>
            <div className="flex items-center gap-3 py-4 last:pb-0"><StudyIcon name="history" size={26} /><dt className="w-28 shrink-0 text-[13px] text-[var(--muted)]">Member since</dt><dd className="text-[14px] font-medium">{formatDate(user.created_at)}</dd></div>
          </dl>
        </section>
        <section className="mt-5 grid gap-4 sm:grid-cols-2" aria-label="Workspace totals">
          <Link href="/my-guides" className="ui-surface ui-surface--interactive flex items-center gap-4 p-5"><span className="ui-icon-frame ui-icon-frame--md ui-icon-frame--primary"><StudyIcon name="guide" size={28} /></span><span><strong className="block text-[24px] leading-none">{summary.guideCount}</strong><span className="mt-1 block text-[13px] text-[var(--muted)]">Guide{summary.guideCount === 1 ? "" : "s"}</span></span></Link>
          <Link href="/library" className="ui-surface ui-surface--interactive flex items-center gap-4 p-5"><span className="ui-icon-frame ui-icon-frame--md ui-icon-frame--source"><StudyIcon name="source" size={28} /></span><span><strong className="block text-[24px] leading-none">{summary.sourceCount}</strong><span className="mt-1 block text-[13px] text-[var(--muted)]">Source{summary.sourceCount === 1 ? "" : "s"}</span></span></Link>
        </section>
