-- Product-3C: owner-scoped Library access paths and private PostgreSQL FTS.

create index if not exists sources_library_recent_idx
  on public.sources(session_id, created_at desc, id desc);

create index if not exists sources_library_kind_recent_idx
  on public.sources(session_id, kind, created_at desc, id desc);

create index if not exists study_guides_private_search_idx
  on public.study_guides using gin (
    to_tsvector('english'::regconfig, coalesce(title, '') || ' ' || coalesce(guide_json::text, ''))
  )
  where archived_at is null and deleted_at is null;

create index if not exists sources_private_search_idx
  on public.sources using gin (
    to_tsvector('english'::regconfig, display_name)
  );

create index if not exists source_spans_private_search_idx
  on public.source_spans using gin (
    to_tsvector('english'::regconfig, text || ' ' || excerpt)
  );
...
begin
  ...
  source_matches as (
    select
      ...
      case
        when pg_catalog.to_tsvector('english'::regconfig, source.display_name) @@ input.query
          then null::text
        else span_match.excerpt
      end as excerpt,
      (1 + greatest(
        pg_catalog.ts_rank(pg_catalog.to_tsvector('english'::regconfig, source.display_name), input.query),
    ...
    where session.owner_user_id = auth.uid()
      ...
      and session.deleted_at is null
      and (
        pg_catalog.to_tsvector('english'::regconfig, source.display_name) @@ input.query
