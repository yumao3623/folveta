...
export function buildSourceSnapshot(input: { sessionId: string; ownerScope: string; title: string; sources: SnapshotSourceInput[] }): V2SourceSnapshot {
  ...
  // Ownership scopes authorization and reuse; it is deliberately excluded from
  // the immutable source/content identity so an account claim preserves it.
  const snapshotHash = hash({ sources: sourceRows.map((source, index) => ({ index, id: source.id, display_name: source.display_name, role: source.role, status: source.status, unit_count: source.unit_count, readable_unit_count: source.readable_unit_count, warnings: source.warnings })), spans: spans.map((span) => ({ id: span.id, source_id: span.source_id, locator: span.locator, ordinal: span.ordinal, content_hash: span.content_hash })) });
...
const v2SectionSchema = z.object({ id: z.string().min(1), title: z.string().min(1), priority: z.enum(["study_first", "study_next", "review_if_time"]), focus_reason: z.string().min(1), explanation: z.array(v2ClaimSchema).min(1), source_refs: z.array(v2SourceReferenceSchema), gaps: z.array(z.object({ code: z.string().min(1), message: z.string().min(1) }).strict()), review_targets: z.array(z.string().min(1)).min(1).optional(), key_concepts: z.array(z.string().min(1)).optional(), definitions: z.array(z.string().min(1)).optional(), processes_relationships: z.array(z.string().min(1)).optional(), common_confusions: z.array(z.string().min(1)).optional(), practice_prompts: z.array(z.string().min(1)).optional() }).strict();
