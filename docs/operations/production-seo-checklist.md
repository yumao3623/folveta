...
- [x] Production Live billing: the public offer is visible (Free: 2 successful Study Guides/month; Folveta Pro: US$12/month for 10), and Live merchant approval, catalog, checkout, webhook, and entitlement enforcement are enabled.
- [x] Checkout, verified billing events, subscription/payment status, billing management, success/cancel/failure, and server-side entitlement enforcement work.
- [x] Duplicate/out-of-order billing events are idempotent and cannot grant incorrect access.
- [x] Public `/pricing`, `/refunds`, and `/contact` pages exist, are reachable, and describe the active Live offer and policy.
- [ ] No hidden result-stage paywall or undisclosed quota remains.

## C. Privacy, security, and operations gates

- [x] Implement and verify scheduled deletion of expired session/account/Guide data and private Storage objects (READY in the inherited Production Readiness closeout).
- [x] Verify user Guide deletion and account deletion semantics, including restore/purge behavior (READY in the inherited Production Readiness closeout).
...
- [x] Free limit/upgrade/checkout/success/cancel/failure/billing-management/entitlement E2E passes in the inherited Paddle Live closeout.
...
- [x] Every sitemap URL matches its canonical and intended index state.
...
- [x] Phase 5 has written sign-off with no open P0/P1 issue; backup/PITR remains an owner risk.
- [x] Launch owner explicitly authorizes public indexing.
